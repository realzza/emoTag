...
class FbankDataset(Dataset):
    def __init__(self, wav_scp, utt2label=None, spk2int=None, fbank_kwargs={}, padding="wrap", cmn=True, tdur=5):
        """
        Params:
            wav_scp         - <utt> <wavpath>
            utt2label       - <utt> <lab>
...
#             spk2int = {x.split()[0]:int(x.split()[1]) for x in open(spk2int)} 
    ...
    def extract_feature(self, h5Dir):
        ...
        hf.close()
        fbankFeat = self.trun_wav(fbankFeat, self.tdur, self.padding)
